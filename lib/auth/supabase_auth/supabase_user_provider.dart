...
class FinalAlumniSupabaseUser extends BaseAuthUser {
  FinalAlumniSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> finalAlumniSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FinalAlumniSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
